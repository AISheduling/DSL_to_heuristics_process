...
SYSTEM_PROMPT = """Ты эксперт-исследователь в области комбинаторных 
задач построения расписаний в условиях ограниченных ресурсов."""
...
def run_experiment(prompt_filepath: Path, output_dir: Path, xl_path: Path,
                   num_runs: int = NUM_RUNS) -> list:
    prompt_text = load_prompt(prompt_filepath)
    dsl_name = prompt_filepath.stem
    results = []
    ...
    for i in range(num_runs):
        ...
        try:
            result = generate_heuristic(prompt_text, run_index=i + 1)
        ...
        with open(json_path, "w", encoding="utf-8") as f:
            json.dump(
                {"dsl_file": dsl_name,
                 "num_runs": num_runs, "runs": results},
...
if __name__ == "__main__":
    ...
    TEXT_DIR = BASE_DIR / "data" / "references" / "text_description_of_projects"
    timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M")
    OUTPUT_DIR = BASE_DIR / "experiments" / "experiment text" / timestamp
    ...
    all_results = {}
    for prompt_file in text_files:
        print(f"{'-' * 55}")
        print(f"Обработка: {prompt_file.name}")
        all_results[prompt_file.name] = run_experiment(
            prompt_filepath=prompt_file,
            output_dir=OUTPUT_DIR,
            xl_path=XL_DIR,
        )

    summary_path = OUTPUT_DIR / "all_text_results.json"

    with open(summary_path, "w", encoding="utf-8") as f:
        json.dump(all_results, f, ensure_ascii=False, indent=2)
    print(f"\nОбщие результаты будут сохранены в: {summary_path}")
