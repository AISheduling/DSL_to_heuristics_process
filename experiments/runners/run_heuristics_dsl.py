...
TIMEOUT_SECONDS = 30  # максимальное время выполнения одной эвристики

THIS_DIR = Path(__file__).resolve().parent  # src/runners/
SRC_DIR = THIS_DIR.parent  # src/
ROOT_DIR = SRC_DIR.parent  # DSL_to_heuristics_process/

EXP_DIR = ROOT_DIR / "experiments" / "experiments_dsl"
DATA_DIR = ROOT_DIR / "data" / "processed"
OUTPUT_DIR = ROOT_DIR / "results" / "dsl"

# Маппинг: стем файла результатов LLM -> файл с данными задачи
CASE_MAP = {
    "Case1_DSL_results": "Case1_PSPLIB_j301_1.json",
    "Case2_DSL_results": "Case2_sch50.json",
    "Case3_DSL_results": "Case3_nonrenewable.json",
    "Case4_DSL_results": "Case4_renewable.json",
    "Case5_DSL_results": "Case5_mp_j90_a2_nr1.json",
}
...
def _run_with_timeout(code: str, jobs, resources):
    """Компилирует и запускает код в отдельном процессе с таймаутом.
    Возвращает результат, None при таймауте, или бросает RuntimeError при ошибке."""
    q = Queue()
...
if __name__ == "__main__":
    ...
    print(f"{'=' * 60}")
    print(f"Эксперимент: experiments_dsl")
